...
if pkg_installed zsh; then
  ...
  prompt_timer 120 "Would you like to add some extra plugins to zsh? (y/n) [default: y]"
  zsh_plugins=false
  ...
  *)
  ...
  if [ "${zsh_plugins}" == false ]; then
    print_log -sec "SHELL" -stat "skipped" "zsh plugins installation skipped..."
  else
